...
void MainWindow::saisieParam()
{
	...
	s3.push_back("à combien seront compressées ou remplis les images\n (si vous choissisez une simulation sur les MNIST ce sera obligatoirement 784)");
...
void MainWindow::quandOnCharge()
{
	...
	QMessageBox(QMessageBox::Information, "Information", "Vous vous appreter a selectionner un fichier contenant une sauvegarde d'un réseau de neurones", QMessageBox::Ok, this).exec();
	...
	if (cheminDacces != "")
	{
		...
		QMessageBox(QMessageBox::Information, "Information", "Ensuite selectionner si vous le souhaiter, le fichier de sauvegarde des stats du reseeau de neurones que vous avez chargé(e)", QMessageBox::Ok, this).exec();
		cheminDacces = openDirectory();
		/*5*/
...
void MainWindow::afficherStructure()
{
	...
	pinceau.drawText(1300, 70, "1 cercle représente " + QString::number(cachee) + " neurones");
...
void MainWindow::quandOnLanceApprentissage()
{
	...
	if (p.nbCouches != 0)
	{
		QMessageBox(QMessageBox::Information, "Information", "Vous avez lancer l'apprentissage, veuillez selectionner le dossier  ", QMessageBox::Ok, this).exec();
		//cheminDacces = openDirectory();
		if (cheminDacces != "")
		{
			//QFileInfo f(cheminDacces);
			...
			if (p.typeSim == 0)
			{
				...
				all = allImage(&lab, chemin, p.nbNeuronesEntree);
			...
			if (p.typeSim == 1)
			{
				chemin = "/Training_Set/MNIST/MNIST_Letter";
			}
			if(p.typeSim == 2)
			{
...
void MainWindow::quandOnSim()
{
	...
	cheminDacces = "/home/massmax/Bureau/Cours/L3/S6/IN608 Projet";//provisoire
	if (p.nbCouches != 0)
	{
	...
	{
		QMessageBox(QMessageBox::Warning, "Attention", "Vous ne pouvez pas faire de simulation car il n'y pas de reseau chargé ou créer sur l'interface ", QMessageBox::Ok, this).exec();
	}
}
...
void MainWindow::resultat(string resultatMap)
{
	...
	res.setText("la reponse est : " + qstr);
	aff->addWidget(&res);
	QFont cara("Times", 24);
	res.setFont(cara);
	res.repaint();
...
void MainWindow::quandOnSaveR()
{
	if(p.nbCouches == 0)
	{
		QMessageBox(QMessageBox::Warning, "Attention", "Vous ne pouvez pas sauvegarder, car il n'y pas de reseau chargé ou créer sur l'interface ", QMessageBox::Ok, this).exec();
		return;
	}
	/*1*/
