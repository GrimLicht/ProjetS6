...
void MainWindow::sendToStructParam()
{
    ...
    else
    {
        ...
        if (ignore)
        {
            ...
            paint = 1;    // permet d'actualiser le paintEvent pour afficher la structure du Reseau
...
QString MainWindow::openDirectory()
{
    ...
    //evite de freeze si l'utilisateur annule l'action
    if (file == "")
    {
...
void MainWindow::quandOnCharge()
{
    ...
    if (cheminDacces != "")
    {
        ...
        temp.reset();
        /*4*/
        QMessageBox(QMessageBox::Information, "Information", "Selectionner, si vous le souhaiter, le fichier de sauvegarde des statistiques du Réseau", QMessageBox::Ok, this).exec();
