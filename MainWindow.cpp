...
QChartView *MainWindow::affGraphe(QString f)
{
	...
	else if(!file.open(QFile::ReadOnly)) QMessageBox::warning(nullptr, f, "Erreur d'ouverture!");
	...
	graphe->resize(700, 350);					  // donne sa taille
...
void MainWindow::saisieParam()
{
	...
	fenetrePara->setWindowTitle("Parametrage création réseau de neurone"); //give title pour fenenetrePara
...
void MainWindow::paintEvent(QPaintEvent *event)
{
	...
	if(paint == true) afficherStructure();
	else
	{
		...
		pinceau.save();
		pinceau.drawText(back, Qt::AlignCenter, "Pas de réseau"); //text ecrit au milieu du rectangle
		pinceau.restore();
	}
}
...
void MainWindow::quandOnCharge()
{
	...
	if(cheminDacces != "")
	{
		...
		msgBox.setText("Selectionnez, si vous le souhaitez, le fichier de sauvegarde des statistiques du Réseau");
...
Parametres MainWindow::chargerRN(QString fichierTXT, vector<MatrixXd> *mPoids, vector<VectorXd> *vBiais)
{
	...
	if(!file.exists()) QMessageBox::warning(nullptr, "Erreur", "Ce fichier n'existe pas");
	...
	while (!data.atEnd())
	{
		...
		for (unsigned int i = 0; i < p.nbCouches - 1; i++) //-1 car la couche de sortie n'a pas de poids
		{ 
			int taille1, taille2;
			if(i == 0)
			{
			...
			while (c != '\n')
			{
				...
				if(c != '\n' && (k < taille2) && (j < taille1))
				{
					...
					else if((i == p.nbCouches - 2) && (k == p.nbNeuronesSortie)) //Dans le cas de la dernière matrice
					{
						k = 0;
						j++;
					}
					else if(k == p.nbNeuronesCache) //Dans le cas des autres matrices
					{
						k = 0;
						j++;
					}
...
void MainWindow::afficherStructure()
{
	...
	if(couches > 2) couche = (double)couches - 2;
	else couche = 1;

	if(couches > 17) couches = 17; //on aura max 15 couches cachees
	couche = couche / (double)(couches - 2);

	if(neuronesEntree > 100) neuronesEntree /= 10;
	...
	if(neuronesCaches > 100) neuronesCaches /= 10;
	...
	if(neuronesCaches) cachee /= (double)neuronesCaches;
	else cachee = 1;
...
void MainWindow::quandOnSim()
{
	if(p.nbCouches != 0)
	{
		...
		/*1*///select le fichier à utiliser
		cheminDacces = openDirectory();
		if(cheminDacces != "")
		{
			/*2*///conversion de l'image choisi en bmp ou MNIST selon type simulation
			VectorXd all;
			if(p.typeSim == 0)
			{
			...
			/*6*///recupère la réponse donnée par le reseau
			string s = toString(reponse, p.typeSim);
			/*7*///affichage du resultat sur l'interface
			resultat(s);
		}
	}
	else QMessageBox(QMessageBox::Warning, "Attention", "Vous ne pouvez pas faire de simulation car aucun Réseau n'a été chargé ou créé", QMessageBox::Ok, this).exec();
}
...
void MainWindow::resultat(string resultatMap)
{
	...
	res.setText("La réponse de votre simulation est : \n" + qstr);
	aff->addWidget(&res);
	QFont cara("Times", 18);
	res.setFont(cara);
	res.repaint();
...
void MainWindow::afficheRN(Parametres param, vector<MatrixXd> *mPoids)
{
	...
	unsigned int largeur, hauteur, bordel; 
...
void MainWindow::quandOnSaveS()
{
   ...
   /*1*/
	...
	/*2*/
	//lance la fonction de sauvegarde du reseau
	if(cheminDacces != "") sauvegardeStat(*rUtilisation, cheminDacces.toStdString());
}
...
MainWindow::~MainWindow() 
{
	...
	/*while(rUtilisation->vCouches.size())
	{
		delete[]rUtilisation->vCouches.back().error;
		rUtilisation->vCouches.pop_back();
	}*/
}